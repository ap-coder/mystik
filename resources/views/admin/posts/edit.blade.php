...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.posts.update", [$post->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="products">{{ trans('cruds.post.fields.products') }}</label>
                <div style="padding-bottom: 4px">
                    <span class="btn btn-info btn-xs select-all" style="border-radius: 0">{{ trans('global.select_all') }}</span>
                    <span class="btn btn-info btn-xs deselect-all" style="border-radius: 0">{{ trans('global.deselect_all') }}</span>
                </div>
                <select class="form-control select2 {{ $errors->has('products') ? 'is-invalid' : '' }}" name="products[]" id="products" multiple>
                    @foreach($products as $id => $products)
                        <option value="{{ $id }}" {{ (in_array($id, old('products', [])) || $post->products->contains($id)) ? 'selected' : '' }}>{{ $products }}</option>
                    @endforeach
                </select>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.post.fields.products_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="faqs">{{ trans('cruds.post.fields.faqs') }}</label>
                <div style="padding-bottom: 4px">
                    <span class="btn btn-info btn-xs select-all" style="border-radius: 0">{{ trans('global.select_all') }}</span>
                    <span class="btn btn-info btn-xs deselect-all" style="border-radius: 0">{{ trans('global.deselect_all') }}</span>
                </div>
                <select class="form-control select2 {{ $errors->has('faqs') ? 'is-invalid' : '' }}" name="faqs[]" id="faqs" multiple>
                    @foreach($faqs as $id => $faqs)
                        <option value="{{ $id }}" {{ (in_array($id, old('faqs', [])) || $post->faqs->contains($id)) ? 'selected' : '' }}>{{ $faqs }}</option>
                    @endforeach
                </select>
                @if($errors->has('faqs'))
                    <div class="invalid-feedback">
                ...
                @endif
                <span class="help-block">{{ trans('cruds.post.fields.faqs_helper') }}</span>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                <label for="meta_desc">{{ trans('cruds.post.fields.meta_desc') }}</label>
                <input class="form-control {{ $errors->has('meta_desc') ? 'is-invalid' : '' }}" type="text" name="meta_desc" id="meta_desc" value="{{ old('meta_desc', $post->meta_desc) }}">
                @if($errors->has('meta_desc'))
                    <div class="invalid-feedback">
                        {{ $errors->first('meta_desc') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.post.fields.meta_desc_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="fb_title">{{ trans('cruds.post.fields.fb_title') }}</label>
                <input class="form-control {{ $errors->has('fb_title') ? 'is-invalid' : '' }}" type="text" name="fb_title" id="fb_title" value="{{ old('fb_title', $post->fb_title) }}">
                @if($errors->has('fb_title'))
                    <div class="invalid-feedback">
                        {{ $errors->first('fb_title') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.post.fields.fb_title_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="twitter_title">{{ trans('cruds.post.fields.twitter_title') }}</label>
                <input class="form-control {{ $errors->has('twitter_title') ? 'is-invalid' : '' }}" type="text" name="twitter_title" id="twitter_title" value="{{ old('twitter_title', $post->twitter_title) }}">
                @if($errors->has('twitter_title'))
                    <div class="invalid-feedback">
                        {{ $errors->first('twitter_title') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.post.fields.twitter_title_helper') }}</span>
            </div>
            <div class="form-group">
